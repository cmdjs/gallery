define("gallery/mergely/2.5.0/mergely-debug", [ "$-debug", "gallery/codemirror/3.1.0/codemirror-debug.js" ], function(require, exports, module) {
    var previousJQuery = this.jQuery;
    this.jQuery = require("$-debug");
    var CodeMirror = require("gallery/codemirror/3.1.0/codemirror-debug.js");
    Mgly = {};
    Mgly.Timer = function() {
        var self = this;
        self.start = function() {
        ...
        diff: function(added, removed) {
            ...
            for (var i = 0; i < changes.length; ++i) {
                ...
                if (change.op != "a") {
                    ...
                    // ignore leading space if not first word
    ...
    jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
        init: function(el, options) {
            this.settings = {
                ...
                _debug: "",
                //scroll,draw,calc,diff,markup,change
                resized: function() {}
            };
            var cmsettings = {
                mode: "text/plain",
            ...
            if (opts) {
                ...
                if (this.settings.hasOwnProperty("sidebar")) {
                    ...
                    if (this.settings.sidebar) {
                        jQuery(this.element).find(".mergely-margin").css({
                            display: "block"
                        });
                    } else {
                        jQuery(this.element).find(".mergely-margin").css({
        ...
        _scrolling: function(editor_name) {
            ...
            if (this.midway == undefined) {
                this.midway = (scroller.height() / 2 + scroller.offset().top).toFixed(2);
            }
            // balance-line
        ...
        _set_top_offset: function(editor_name1) {
            ...
            this.draw_top_offset = .5 - top_offset;
            return true;
        },
        _calculate_offsets: function(editor_name1, editor_name2, changes) {
            ...
            for (var i = 0; i < changes.length; ++i) {
                ...
                if (this.editor[editor_name1].getOption("lineWrapping") || this.editor[editor_name1].getOption("lineWrapping")) {
                    ...
                    var tls = this.editor[editor_name1].cursorCoords({
                        line: llf,
                        ch: 0
                    }, "page");
                    ...
                    var tle = this.editor[editor_name1].cursorCoords({
                        line: llt,
                        ch: 0
                    }, "page");
                    ...
                    var tls = this.editor[editor_name2].cursorCoords({
                        line: rlf,
                        ch: 0
                    }, "page");
                    ...
                    var tle = this.editor[editor_name2].cursorCoords({
                        line: rlt,
                        ch: 0
                    }, "page");
        ...
        _markup_changes: function(editor_name1, editor_name2, changes) {
            ...
            for (var i = 0; this.settings.lcs && i < changes.length; ++i) {
                ...
                if (change["op"] == "d") {
                    ...
                    for (var j = llf, k = rlf, p = 0; j >= 0 && j <= llt || k >= 0 && k <= rlt; ++j, 
                    ++k) {
                        if (k + p > rlt) {
            ...
            jQuery(".merge-button").on("click", function(ev) {
                ...
                var line = {
                    lhs: ed["lhs"].lineInfo(llt),
                    rhs: ed["rhs"].lineInfo(rlt)
                };
        ...
        _draw_diff: function(editor_name1, editor_name2, changes) {
            ...
            for (var i = 0; i < changes.length; ++i) {
                ...
                var cy = lhs_y_start + (lhs_y_end + 1 - lhs_y_start) / 2;
                var dx = this.draw_rhs_max - this.draw_rhs_width;
                var dy = rhs_y_start + (rhs_y_end + 1 - rhs_y_start) / 2;
                ctx.moveTo(cx, cy);
                if (cy == dy) {
                    ctx.lineTo(dx, dy);
                } else {
                    // fancy!
                    ctx.bezierCurveTo(cx + 12, cy - 3, // control-1 X,Y
