define("gallery/codemirror/3.1.0/codemirror-debug", [], function(require, exports, module) {
    ...
    window.CodeMirror = function() {
        ...
        function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
            ...
            cm.doc.iter(from, to, function(line) {
                ...
                if (lineIsHidden(cm.doc, line)) {
                    ...
                    if (line.widgets && cur.previousSibling) for (var i = 0; i < line.widgets.length; ++i) if (line.widgets[i].showIfHidden) {
        ...
        function buildLineElement(cm, line, lineNo, dims, reuse) {
            ...
            if (line.bgClass) wrap.insertBefore(elt("div", " ", line.bgClass + " CodeMirror-linebackground"), wrap.firstChild);
        ...
        function updateSelectionRange(cm) {
            ...
            if (sel.from.line == sel.to.line) {
                ...
                if (singleLine) {
                    ...
                    if (sel.from.ch) // Draw the first line of selection.
        ...
        function measureLine(cm, line) {
            ...
            for (var i = 0; i < cache.length; ++i) {
                var memo = cache[i];
                if (memo.text == line.text && memo.markedSpans == line.markedSpans && display.scroller.clientWidth == memo.width && memo.classes == line.textClass + "|" + line.bgClass + "|" + line.wrapClass) return memo.measure;
            }
            ...
            if (cache.length == 16) cache[++display.measureLineCachePos % 16] = memo; else cache.push(memo);
            return measure;
        }
        ...
        function cursorCoords(cm, pos, context, lineObj, measurement) {
            ...
            for (var i = 0; i < order.length; ++i) {
                ...
                if (left == ch) {
                    ...
                    if (i && part.level < (nb = order[i - 1]).level) here = get(nb.level % 2 ? nb.from : nb.to - 1, true); else here = get(rtl && part.from != part.to ? ch - 1 : ch);
                    if (rtl == linedir) main = here; else other = here;
                } else if (right == ch) {
                    var nb = i < order.length - 1 && order[i + 1];
                    if (!rtl && nb && nb.from == nb.to) continue;
                    if (nb && part.level < nb.level) here = get(nb.level % 2 ? nb.to - 1 : nb.from); else here = get(rtl ? ch : ch - 1, true);
                    if (rtl == linedir) main = here; else other = here;
                }
            }
        ...
        function coordsChar(cm, x, y) {
            ...
            for (;;) {
                ...
                if (merged && found.ch >= mergedPos.from.ch) lineNo = mergedPos.to.line; else return found;
            }
        }
        function coordsCharInner(cm, lineObj, lineNo, x, y) {
            ...
            function getX(ch) {
                ...
                wrongLine = true;
                if (innerOff > sp.bottom) return Math.max(0, sp.left - cWidth); else if (innerOff < sp.top) return sp.left + cWidth; else wrongLine = false;
                return sp.left;
            }
        ...
        function readInput(cm) {
            ...
            if (text.length > 1e3) input.value = cm.display.prevInput = ""; else cm.display.prevInput = text;
            if (withOp) endOperation(cm);
        ...
        function resetInput(cm, user) {
            ...
            if (!posEq(doc.sel.from, doc.sel.to)) {
                ...
                if (minimal) cm.display.input.value = "-"; else cm.display.input.value = selected || cm.getSelection();
        ...
        function calculateScrollPos(cm, x1, y1, x2, y2) {
            ...
            if (y1 < screentop) result.scrollTop = atTop ? 0 : Math.max(0, y1); else if (y2 > screentop + screen) result.scrollTop = (atBottom ? docBottom : y2) - screen;
        ...
        CodeMirror.prototype = {
            ...
            addLineClass: operation(null, function(handle, where, cls) {
                return changeLine(this, handle, function(line) {
                    ...
                    if (!line[prop]) line[prop] = cls; else if (new RegExp("\\b" + cls + "\\b").test(line[prop])) return false; else line[prop] += " " + cls;
                    return true;
                });
            }),
            ...
            moveV: operation(null, function(dir, unit) {
                ...
                if (unit == "page") this.display.scrollbarV.scrollTop += charCoords(this, target, "div").top - pos.top;
        ...
        function addLineWidget(cm, handle, node, options) {
            ...
            changeLine(cm, handle, function(line) {
                ...
                if (!lineIsHidden(cm.doc, line) || widget.showIfHidden) {
                    ...
                    if (aboveVisible) cm.curOp.updateScrollPos = {
        ...
        function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
            for (var i = 0; i < text.length; ++i) {
                ...
                if (ch >= "���" && ch < "���" && i < text.length - 1) {
                    ch = text.slice(i, i + 2);
                    ++i;
                } else if (i && builder.cm.options.lineWrapping && spanAffectsWrapping.test(text.slice(i - 1, i + 1))) {
                    builder.pre.appendChild(elt("wbr"));
                }
                builder.measure[builder.pos] = buildToken(builder, ch, style, start && startStyle, i == text.length - 1 && endStyle);
        ...
        function insertLineContent(line, builder, styles) {
            var spans = line.markedSpans;
            if (!spans) {
                for (var i = 1; i < styles.length; i += 2) builder.addToken(builder, styles[i], styleToClass(styles[i + 1]));
                return;
            }
        ...
        function updateDoc(doc, change, markedSpans, selAfter, estimateHeight) {
            ...
            if (from.ch == 0 && to.ch == 0 && lastText == "") {
                ...
                if (text.length == 1) {
                    updateLine(firstLine, firstLine.text.slice(0, from.ch) + lastText + firstLine.text.slice(to.ch), lastSpans, estimateHeight);
                } else {
                ...
                updateLine(firstLine, firstLine.text.slice(0, from.ch) + text[0] + lastLine.text.slice(to.ch), spansFor(0), estimateHeight);
        ...
        function addToHistory(doc, change, selAfter, opId) {
            ...
            if (cur && (hist.lastOp == opId || hist.lastOrigin == change.origin && change.origin && (change.origin.charAt(0) == "+" && hist.lastTime > time - 600 || change.origin.charAt(0) == "*"))) {
                ...
                if (hist.dirtyCounter < 0) // The user has made a change after undoing past the last clean state. 
                // We can never get back to a clean state now until markClean() is called.
                hist.dirtyCounter = NaN; else hist.dirtyCounter++;
            }
            hist.lastTime = time;
            hist.lastOp = opId;
        ...
        function removeChildren(e) {
            ...
            if (!ie) e.innerHTML = ""; else while (e.firstChild) e.removeChild(e.firstChild);
            return e;
        }
